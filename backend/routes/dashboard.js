...
router.get('/employee', authenticateToken, authorizeRoles('employee'), async (req, res) => {
  try {
    ...
    const [assets, gatePasses, openReports, pendingRequests] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM gate_passes WHERE employee_id = $1 AND status IN ('pending_head','pending_owner_dept','pending_procurement','active')", [userId]),
      pool.query("SELECT COUNT(*) FROM issue_reports WHERE reported_by = $1 AND status NOT IN ('resolved','rejected')", [userId]),
      pool.query("SELECT COUNT(*) FROM asset_requests WHERE assigned_to = $1 AND status IN ('pending_owner_dept','pending_procurement','partially_fulfilled')", [userId]),
    ]);
    res.json({
...
router.get('/head', authenticateToken, authorizeRoles('head'), async (req, res) => {
  try {
    ...
    const [assets, gatePasses, ownerGatePasses, ownerRequests, ownerReports, reports, employees, deptRequests, fulfilledRequests] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM asset_requests WHERE department_id = $1 AND status IN ('pending_owner_dept','pending_procurement')", [departmentId]),
...
router.get('/procurement', authenticateToken, authorizeRoles('procurement'), async (req, res) => {
  try {
    const [total, available, assigned, requests, gatePasses, reports, lowStock, fulfilled] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM asset_requests WHERE status = 'pending_procurement'"),
      pool.query("SELECT COUNT(*) FROM gate_passes WHERE status = 'pending_procurement'"),
      pool.query("SELECT COUNT(*) FROM issue_reports WHERE status = 'pending_procurement'"),
...
router.get('/hr', authenticateToken, authorizeRoles('hr'), async (req, res) => {
  try {
    const [depts, employees, active, inactive] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM users u JOIN roles r ON u.role_id = r.id WHERE r.name IN ('employee','head')"),
