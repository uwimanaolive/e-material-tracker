...
export const InventoryReports = () => {
  ...
  const loadReports = async () => {
    try {
      const [p, h] = await Promise.all([
        issueReportsApi.getInventoryPending(),
        issueReportsApi.getInventoryHistory(),
      ]);
      setPending(p);
      setHistory(h);
    } catch {
  ...
  const handleResolve = async () => {
    ...
    try {
      await issueReportsApi.inventoryAction(selectedReport.id, {
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="pending">
        ...
        <TabsContent value="pending">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><AlertTriangle className="w-5 h-5" />Awaiting Inventory Department</CardTitle></CardHeader>
