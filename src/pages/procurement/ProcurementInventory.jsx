...
export const ProcurementInventory = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setDepartments(deptsData.filter((d) => d.name !== "Procurement" && d.name !== "Human Resources"));
    } catch (error) {
  ...
  const handleSave = async () => {
    if (!form.name || !form.category || !form.serial_number) {
      toast.error("Name, category, and serial number are required");
      return;
    }
    ...
    try {
      const payload = {
        ...
        store_department: form.store_department || null,
      };
                ...
                <TableBody>
                  {assets.map((asset) => (
                    <TableRow key={asset.id}>
                      ...
                      <TableCell>{asset.store_department_name || "General"}</TableCell>
                      <TableCell>{asset.current_location || asset.current_department || "Procurement Store"}</TableCell>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            ...
            <div>
              <Label>Store Pool (Department)</Label>
              <Select value={form.store_department || "general"} onValueChange={(v) => setForm({ ...form, store_department: v === "general" ? "" : v })}>
              ...
              <p className="text-xs text-muted-foreground mt-1">Reserve this item for a specific department in the procurement store</p>
            </div>
            <div>
              <Label>Notes</Label>
